...
import { betIndex, currencies, fxSeries, inflation, rates } from "@/lib/mock-data";
import { ArrowRight, TrendingUp } from "lucide-react";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Acasă — Economia României" },
      { name: "description", content: "Vedere de ansamblu: curs BNR, BET, ROBOR, inflație, dobânda BNR." },
    ],
  }),
  component: HomePage,
});
...
function HomePage() {
  ...
  return (
    <div className="space-y-10">
      {/* Hero */}
      <section className="rounded-3xl bg-gradient-to-br from-pastel-blue/40 via-pastel-lavender/30 to-pastel-mint/40 p-8 sm:p-12">
        <div className="max-w-2xl">
          ...
          <h1 className="mt-5 text-4xl sm:text-5xl font-semibold tracking-tight text-foreground">
            Economia României, <span className="text-foreground/60">pe înțelesul tuturor.</span>
          </h1>
          <p className="mt-4 text-[15px] sm:text-base text-muted-foreground leading-relaxed">
            Curs valutar, indicele bursier, dobânzi și inflație — toate într-un singur loc, explicate simplu și actualizate zilnic.
          </p>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <MetricCard
            label="Indicele BET"
            value={betIndex.value}
            prev={betIndex.prev}
            current={betIndex.value}
            digits={2}
            series={betIndex.series}
            tone="butter"
          ...
          <MetricCard
            ...
            unit="%"
            tone="lavender"
            explainer="Dobânda la care băncile își împrumută bani între ele pe 3 luni. Influențează ratele la creditele în lei."
          />
          <MetricCard
            label="Dobânda BNR"
            value={rates.bnrPolicy}
            unit="%"
            tone="mint"
            explainer="Dobânda de politică monetară a Băncii Naționale — instrumentul principal pentru controlul inflației."
          />
          <MetricCard
            label="Inflație anuală"
            value={inflation.yearly}
            unit="%"
            tone="peach"
          ...
          />
          <MetricCard
            label="Inflație lunară"
            value={inflation.monthly}
            unit="%"
            tone="sand"
      ...
      <section className="rounded-3xl bg-card soft-shadow p-6 sm:p-8 flex flex-col sm:flex-row gap-6 items-start">
        <div className="h-12 w-12 rounded-2xl bg-pastel-mint/60 flex items-center justify-center shrink-0">
